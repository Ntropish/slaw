...
window.customElements.define('wam-knob', WamKnob)

export default class WamKnob extends HTMLElement {
  constructor() {
    super()
  ...
  connectedCallback() {
    ...
    if (this.hasAttribute('strip')) img.src = this.getAttribute('strip')
  ...
  onmouse(e) {
    if (e.type == 'mousedown') {
      ...
      if (dy != 0) {
        ...
        if (this.step) {
          this._.acc += dy / 100
          if (Math.abs(this._.acc) >= this.step) {
            let sign = this._.acc > 0 ? 1 : -1
            v = this._.value + sign * this.step
            this._.acc = 0
          }
        } else v = parseFloat((this._.value + dy / 100).toFixed(2))
        if (v < 0) v = 0
        else if (v > 1) v = 1
        if (v != this._.value) {
          this.value = v
